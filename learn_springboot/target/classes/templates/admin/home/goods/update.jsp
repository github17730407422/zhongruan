...
<div class="breadcrumb-line">
    <ul class="breadcrumb">
        ...
        <li class="active">修改商家商品</li>
    </ul>
</div>
<div class="block s-form">
    <form class="form-horizontal" name="goodsForm" id="goodsForm" action="<s:url value="/admin/goodsUpdate.do" includeParams='none'/>" role="form" method="post" enctype="multipart/form-data">
        <div class="panel panel-default ">

            <div class="panel-body">
            	<div class="form-group"><label class="col-sm-2 control-label text-right">城市:<font style="color: red;">*</font></label>
                    <div class="col-sm-10">
                    <select class="form-control" id="cityId" name="goodsInfo.cityId" onchange="getBusinessSimpList()" disabled="disabled">
                ...
                <div class="form-group "><label class="col-sm-2 control-label text-right">所属商家:<font style="color: red;">*</font></label>
                    <div class="col-sm-10">
                        <select class="form-control" id="businessId" name="goodsInfo.businessId" disabled="disabled">
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group "><label class="col-sm-2 control-label text-right">商品描述:</label>
                    <div class="col-sm-10">
                        <input type="text" class="form-control" id="descriptionSimp" name="goodsInfo.descriptionSimp" value="<s:property value="goodsInfo.descriptionSimp" />">
                    </div>
                </div>
                <hr/>
                ...
                <input type="hidden" id="descriptionType" name="goodsInfo.descriptionType" value="<s:property value="goodsInfo.descriptionType" />">
                <div class="form-group "><label class="col-sm-2 control-label text-right">&nbsp;</label>
                    <input type="radio" name="descriptionTypeButton" value="0" onClick="changeDescriptionType(0)"/>系统默认模式<br />
                    <input type="radio" name="descriptionTypeButton" value="1" onClick="changeDescriptionType(1)"/>商家自定义模式
                </div>
                
                <div class="form-group ">
                    <span id="editorSpan">
                        <textarea id="description" style="width:750px;height:500px;" name="goodsInfo.description"  class="ckeditor"><s:property value="goodsInfo.description" /></textarea>
                    </span>
                </div>
                
                <div class="form-actions text-right">
                    <input type="hidden" id="bId" name="bId" value="<s:property value="goodsInfo.businessId" />">
                    <input type="hidden" id="goodsId" name="goodsInfo.id" value="<s:property value="goodsInfo.id" />">
                    <!-- 保存查询商品的参数 start -->
                    <input type="hidden"  name="paramGoodsInfo.cityId" value="<s:property value="paramGoodsInfo.cityId" />">
                    <input type="hidden" name="paramGoodsInfo.status" value="<s:property value="paramGoodsInfo.status" />">
                    <input type="hidden" name="paramGoodsInfo.businessName" value="<s:property value="paramGoodsInfo.businessName" />">
                    <input type="hidden"  name="paramGoodsInfo.goodsName" value="<s:property value="paramGoodsInfo.goodsName" />">
                        <!-- 保存查询商品的参数 end -->
...
<input type="hidden" id="deletedSpecIDs" name="deletedSpecIDs" value="" />
...
function initAttributeSpecArray() {
	...
	//数组
<s:iterator value="attributeList" id="goodsAttributeInfo" status="rowstatus">
	attributeIdArray[<s:property value="#rowstatus.index" />] = '<s:property value="#goodsAttributeInfo.id" />';
	attributeNameArray[<s:property value="#rowstatus.index" />] = uncode('<s:property value="#goodsAttributeInfo.attributeName" />');
	if ('<s:property value="#goodsAttributeInfo.attributeName" />' == '') {
		var rowId = "attribute_row_" + '<s:property value="#rowstatus.index" />';
		document.getElementById(rowId).style.display = 'none';
	}
</s:iterator>

<s:iterator value="specList" id="goodsSpecInfo" status="rowstatus">
    for (var i = 0; i < attributeSize; i++) {
    	if (attributeIdArray[i] == '<s:property value="#goodsSpecInfo.attributeId" />') {
    		var j = 0;
    		for (var j = 0; j < specSize; j++) {
    			if (specArray[i][j] == "") {
        			var id = '<s:property value="#goodsSpecInfo.id" />';
        			var specName = uncode('<s:property value="#goodsSpecInfo.specName" />');
        			var tmpSpecPrice = parseFloat('<s:property value="#goodsSpecInfo.specPrice" />');
        			var specSign = tmpSpecPrice < 0 ? "-" : "+";
        			var specPrice = Math.abs(tmpSpecPrice);
        			specArray[i][j] = id + splitStr + specName + splitStr + specSign + splitStr + specPrice;
...
//top.dialog_alert("-0-: " + specArray[0][0] + "-1-: " + specArray[0][1] + "-2-: " + specArray[1][0] + "-3-: " + specArray[1][1]);

	//页面
	for (var i = 0; i < attributeSize; i++) {
		if (attributeIdArray[i] != -1) {
			var rowId = "attribute_row_" + i;
			var labelId = "attribute_label_" + i;
			document.getElementById(labelId).innerHTML = attributeNameArray[i];
...
//	alert(description);
	...
	var param = {"goodsInfo.goodsName":goodsName, "goodsInfo.id":goodsId, "goodsInfo.businessId":businessId};
...
function resetGoods() {
	location.href="<s:url value='/admin/goodsList.do' includeParams='none'/>?paramGoodsInfo.cityId="+$("input[name='paramGoodsInfo.cityId']").val()
			+"&paramGoodsInfo.status="+$("input[name='paramGoodsInfo.status']").val()
		+"&paramGoodsInfo.businessName="+$("input[name='paramGoodsInfo.businessName']").val()
		+"&paramGoodsInfo.goodsName="+$("input[name='paramGoodsInfo.goodsName']").val();
}
...
function deleteAttribute(pageIndex) {
	top.dialog_confirm("您确认删除该属性及其所有规格选项吗？", function(){
    	if (attributeIdArray[pageIndex] != -1) {
    		var str = $('#deletedAttributeIDs').val() + splitStr;
    		$('#deletedAttributeIDs').val(str + attributeIdArray[pageIndex]);
    	}
...
function showSpecDiv(pageIndex) {
	...
	for (var i = 0; i < specSize; i++) {
		...
		if (str != "") {
			...
			document.getElementById("spec_price_" + i).innerHTML = strArray[2] + roundNumber(strArray[3], 2) + "￥";
...
function deleteSpec() {
	...
	top.dialog_confirm("您确认删除该规格选项吗？", function(){
		var specId = specArray[pageIndex][selectedRowNum].substring(0, specArray[pageIndex][selectedRowNum].indexOf(splitStr));
		if (specId != "-1") {
			var str = $('#deletedSpecIDs').val() + splitStr;
			$('#deletedSpecIDs').val(str + specId);
		}
...
//	}
	...
	if ((parseFloat(price) + parseFloat(specSign + trim(specPrice))) <= 0) {
		document.getElementById("specMsg").innerHTML = "波动价格不能使得商品价格小于或者等于零，请重新设置波动价格。";
		return false;
	}
	...
	if (specOperationType == 1) {
		...
		//数组
		var specId = specArray[pageIndex][selectedRowNum].substring(0, specArray[pageIndex][selectedRowNum].indexOf(splitStr));
		specArray[pageIndex][selectedRowNum] = specId + splitStr + specName + splitStr + specSign + splitStr + roundNumber(specPrice, 2);
...
function initDescriptionInfo() {
	if ('<s:property value="goodsInfo.descriptionType" />' == 0) {
		document.goodsForm.descriptionTypeButton[0].checked = true;
		document.getElementById("editorSpan").style.display = "none";
	} else {
		document.goodsForm.descriptionTypeButton[1].checked = true;
		document.getElementById("editorSpan").style.display = "";
	}
}
