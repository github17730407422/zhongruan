...
<body class="sub-page">
	...
	<form class="form-inline tab-pane" role="form" id="listForm"
		name="listForm"
		action="<s:url value="/admin/orderList.do" includeParams='none'/>"
		method="post">
		...
		<div class="block search">
			<div style="margin-bottom: 5px">
				...
				<div class="form-group">
					<label class="">收货地址:</label> <input type="text"
						class="form-control" id="searchOrderaddr" name="searchOrderaddr"
						value="<s:property value="searchOrderaddr"/>" />
				</div>
				<div class="form-group">
					<label class="">状态:</label> <select class="form-control"
						name="searchOrderStatus" id="searchOrderStatus">
						<option value="-1">请选择</option>
						<option value="0"
							<s:if test="searchOrderStatus == 0">selected="selected"</s:if>>
							未支付</option>
						<option value="1"
							<s:if test="searchOrderStatus == 1">selected="selected"</s:if>>
							已下单</option>
						<option value="2"
							<s:if test="searchOrderStatus == 2">selected="selected"</s:if>>
							已派单</option>
						<option value="4"
							<s:if test="searchOrderStatus == 4">selected="selected"</s:if>>
							派送中</option>
						<option value="5"
							<s:if test="searchOrderStatus == 5">selected="selected"</s:if>>
							已完成</option>
						<option value="6"
							<s:if test="searchOrderStatus == 6">selected="selected"</s:if>>
							已取消</option>
					</select>
					<div class="form-group">
						<label class="">配送员:</label> <input type="text"
							class="form-control" id="searchSendName" name="searchSendName"
							value="<s:property value="searchSendName"/>" />
					</div>
				</div>
			...
			<div class="datatable-tasks">
				<table class="table table-bordered">
					...
					<tbody>
						<s:if test="pagination.hasData == true">
							<s:iterator value="pagination.dataList" status="rowstatus">
								<tr>
									...
									<td class="task-desc"><p title="<s:property value="user_address"/>"><s:property value="%{formatName(user_address,17)}"/></p>
									</td>
									...
									<td class="task-desc">取餐员：<s:if test="take_name==null">无 </s:if>
										<s:else>
											<s:property value="take_name" />
										</s:else><br> 送餐员：<s:if test="send_name==null">无 </s:if><s:else>
												<s:property value="send_name" />
											</s:else><br>
									</td>
									<td class="text-center">
										<div class="btn-group">
											<s:if test="status != 0 && status != 5 && status != 6">
												<button type="button" class="btn btn-default"
													data-toggle="modal" data-target="#peisong"
													onclick="showDeliveryBox('<s:property value="id"/>', '<s:property value="take_id"/>', '<s:property value="send_id"/>', '<s:property value="take_center_id"/>', '<s:property value="send_center_id"/>', '<s:property value="status"/>','<s:property value="%{formatDateTime(creat_time)}" />')">
													<i class="fa fa-pencil fa-lg"></i>配送
												</button>
											</s:if>
											<s:if test="status != 5 && status != 6">
											<button type="button" class="btn btn-default"
													data-toggle="modal" data-target="#cancelOrder"
													onclick="showCancelBox(<s:property value="id"/>)">
													<i class="fa fa-trash-o fa-lg"></i>取消
											</button>
											</s:if>
						...
						<s:else>
							<tr>
								<td colspan="8" align="center">没有搜索到符合条件的记录，请重新搜索！</td>
							</tr>
						</s:else>
					</tbody>
				</table>
	...
	<form id="assignMemberForm" name="assignMemberForm"
		action="<s:url value="/admin/orderDeal.do" includeParams='none'/>"
		method="post">
		<input type="hidden" id="orderId" name="orderId" value="" /> <input
			type="hidden" id="takeId" name="takeId" value="" /> <input
			type="hidden" id="takeCenterId" name="takeCenterId" value="" /> <input
			type="hidden" id="sendId" name="sendId" value="" /> <input
			type="hidden" id="sendCenterId" name="sendCenterId" value="" /> <input 
			type="hidden" id="orderStatus" name="orderStatus" value=""/> <input 
			type="hidden" id="ordertTime" name="ordertTime" value=""/>
	</form>

	<form id="cancelOrderForm" name="cancelOrderForm"
		action="<s:url value="/admin/orderCancel.do" includeParams='none'/>"
		method="post">
		<input type="hidden" id="cancelOrderId" name="cancelOrderId" value="" />
		<input type="hidden" id="orderCancelReason" name="orderCancelReason"
			value="" />
	</form>
	
	<form id="updatePriceForm" name="updatePriceForm" 
		action="<s:url value="/admin/updatePrice.do" includeParams='none'/>"
		method="post">
...
//	$('#updatePriceForm').submit();
	...
	var act = "<s:url value='/admin/updatePrice.do' includeParams='none'/>";
...
function showDeliveryBox(orderId, take_id, send_id, take_center_id, send_center_id, status, creat_time) {
	$("#ordertTime").val(creat_time);
	$("#orderId").val(orderId);
